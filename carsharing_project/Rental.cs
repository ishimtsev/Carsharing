...
namespace carsharing_project
{
	public partial class Rental : Form
	{
		...
		{
			AddRental form = new AddRental(dataGridView1.CurrentRow.Cells[0].Value.ToString(), dataGridView1.CurrentRow.Cells[1].Value.ToString(), dataGridView1.CurrentRow.Cells[2].Value.ToString(), dataGridView1.CurrentRow.Cells[3].Value.ToString(), dataGridView1.CurrentRow.Cells[6].Value.ToString(), dataGridView1.CurrentRow.Cells[7].Value.ToString(), dataGridView1.CurrentRow.Cells[14].Value.ToString(), dataGridView1.CurrentRow.Cells[13].Value.ToString());
			form.FormClosed += (s, args) => BindData();
			form.Show();
		}
		...
		private void BindData()
		{
			...
			try
			{
				using (NpgsqlConnection cn = new NpgsqlConnection(Connection.str))
				{
					...
					dataGridView1.Columns[13].Visible = false; //regID
					cn.Close();
		...
		private void DistanceButton1_Click(object sender, EventArgs e)
		{
			...
			//		MessageBox.Show("Расстояние до автомобиля равно " + distance + " м");
			//		cn.Close();
			//	}
			//}
			//catch (Exception er)
		...
		private void MapButton1_Click(object sender, EventArgs e)
		{
			RentalMap form = new RentalMap(dataGridView1.CurrentRow.Cells[0].Value.ToString());
			//form.FormClosed += (s, args) => BindData();
			form.Show();
		}
	}
